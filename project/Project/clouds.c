...
CloudSystem* createCloudSystem(float baseHeight) {
    CloudSystem* cs = (CloudSystem*)malloc(sizeof(CloudSystem));
    if (!cs) return NULL;
    ...
    for (int i = 0; i < cs->cloudCount; i++) {
        Cloud* cloud = &cs->clouds[i];
        
        cloud->x = ((float)rand()/RAND_MAX - 0.5f) * TERRAIN_SCALE * 1.2f;
        cloud->z = ((float)rand()/RAND_MAX - 0.5f) * TERRAIN_SCALE * 1.2f;
...
void updateClouds(CloudSystem* cs, float deltaTime, float dayTime) {
    for (int i = 0; i < cs->cloudCount; i++) {
        ...
        if (cloud->x > TERRAIN_SCALE) {
            cloud->x = -TERRAIN_SCALE;
            cloud->z = ((float)rand()/RAND_MAX - 0.5f) * TERRAIN_SCALE;
